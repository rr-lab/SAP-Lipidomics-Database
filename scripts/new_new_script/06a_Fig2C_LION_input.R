...
#   * only species detected in both trials are included (164 of 238). A species
...
# LION reads a plain CSV, so a species name containing a comma splits into extra
# fields and shifts that row. Two oxylipin names do
# ("8(9)-Epoxy-5Z,11Z,14Z-eicosatrienoic acid, methyl ester" and "9,12,13,TriHODE").
# Only those names are quoted, so every other row stays byte-identical to the
# file LION has already accepted.
nm <- rownames(vals)
nm <- ifelse(grepl(",", nm, fixed = TRUE),
             paste0('"', gsub('"', '""', nm, fixed = TRUE), '"'), nm)

out <- rbind(c("", groups),
             c("", ids),
             cbind(nm, format(vals, scientific = TRUE, digits = 8, trim = TRUE)))
...
unmappable <- c("MGDG", "DGDG", "SQDG", "LPE")
