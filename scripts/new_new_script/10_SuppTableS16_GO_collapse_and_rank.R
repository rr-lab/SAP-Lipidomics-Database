...
# ---- LaTeX table of the collapsed loci ---------------------------------------
# Replaces the per-term longtable, which listed the same locus several times.
tex_file <- file.path(REPO, "tables/chapter2_go_enrichment.tex")
dir.create(dirname(tex_file), recursive = TRUE, showWarnings = FALSE)

esc <- function(x) gsub("_", "\\\\_", gsub("&", "\\\\&", gsub("%", "\\\\%", x)))
body <- loci %>%
  mutate(Fold = ifelse(Fold_min == Fold_max, sprintf("%.1f", Fold_max),
                       sprintf("%.1f--%.1f", Fold_min, Fold_max)),
         line = sprintf("%s & %s & %s & %s & %s & %d & %d & %d & %s & %s \\\\",
                        Condition, esc(Classes), Ontologies, esc(Term), "",
                        Genes, Background, Intervals, Fold, format(signif(q_LD, 2)))) %>%
  mutate(line = sub(" &  & ", " & ", line)) %>% pull(line)

hdr <- c(
"% Auto-generated by 10_SuppTableS16_GO_collapse_and_rank.R -- do not edit by hand.",
...
"\\midrule",
"\\endhead")
writeLines(c(hdr, body, "\\bottomrule", "\\end{longtable}"), tex_file)
message("Saved: ", tex_file, "  (", length(body), " rows)")
