...
# batch mode: 33_...py --layer individual --nperm 2000 --seed 3 --batch out.json
ARG = {sys.argv[i]: sys.argv[i+1] for i in range(1, len(sys.argv)-1, 2)}
BATCH = ARG.get("--batch")
if BATCH:
    N_PERM = int(ARG.get("--nperm", 2000)); SEED = int(ARG.get("--seed", 1))
LAYERS = (ARG["--layer"],) if "--layer" in ARG else ("individual", "sumratio")
...
for r in csv.DictReader(open(ANNOT), delimiter="\t"):
    gene_terms[r["GeneID"].strip()].update(parse_go_bp(r.get("GO_MF", "")))
...
BATCH_OUT = {}
for L in LAYERS:
    src = f"{GO}/GO_MF_enrichment_by_lipid_class_{L}_LD.tsv"
    ...
    for (cond, cl), grp in {(c, k): [r for r in rows if r["condition"] == c and r["lipid_class"] == k]
        ...
        for r in grp:
            ...
            if BATCH:
                BATCH_OUT[f"{L}|{cond}|{cl}|{r['go_term']}"] = [exceed, N_PERM, float(null.sum())]
            p = (exceed + 1) / (N_PERM + 1)
    ...
    path = f"{GO}/GO_MF_enrichment_by_lipid_class_{L}_LDaware.tsv"
...
if BATCH:
    json.dump(BATCH_OUT, open(BATCH, "w"))
    print(f"batch written: {BATCH}  ({len(BATCH_OUT)} terms, {N_PERM} perms, seed {SEED})")
