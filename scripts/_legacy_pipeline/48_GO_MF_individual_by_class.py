...
"""GO molecular-function enrichment stratified by lipid class, individual-lipid GWAS.

Companion to the GO-BP analysis. MF and BP are corrected separately, never pooled:
the two ontologies annotate overlapping gene sets, so pooling them double-counts
the same locus and makes the BH denominator meaningless.
...
with open(ANNOT, newline="") as handle:
    ...
    for row in reader:
        ...
        gene_terms[gene].update(parse_go_bp(row.get("GO_MF", "")))
...
for condition in sorted(class_genes):
    for lipid_class in sorted(class_genes[condition]):
        ...
        summary.append({
            ...
            "GO_MF_testable_genes": len(testable),
        })
...
result_file = f"{OUT}/GO_MF_enrichment_by_lipid_class_individual_LD.tsv"
...
summary_file = f"{OUT}/GO_MF_enrichment_by_lipid_class_individual_LD_summary.tsv"
with open(summary_file, "w", newline="") as handle:
    fields = [
        "condition", "lipid_class", "individual_phenotypes", "candidate_genes",
        "GO_MF_testable_genes", "GO_terms_tested", "BH_significant_terms",
    ]
...
for row in summary:
    print(
        ...
        f"{row['GO_MF_testable_genes']} GO-testable, "
