...
# Assemble Figure 1
# ─────────────────────────────────────────────────────────────────────────────
fig1a_path <- "fig/main/individual_figs/Fig1A_working_pipeline.png"
if (file.exists(fig1a_path) && requireNamespace("png", quietly = TRUE)) {
  fig1a_raster <- tryCatch(png::readPNG(fig1a_path), error = function(e) NULL)
  if (!is.null(fig1a_raster)) {
    fig1a_panel <- ggplot() + annotation_custom(grid::rasterGrob(fig1a_raster, interpolate = TRUE),
                                                 xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = Inf) +
      coord_cartesian(expand = FALSE) + theme_void()
  } else {
    fig1a_panel <- ggplot() + annotate("text", x = 0.5, y = 0.5, label = "Fig 1A\n(placeholder)", size = 5) + theme_void()
  }
} else {
  fig1a_panel <- ggplot() + annotate("text", x = 0.5, y = 0.5, label = "Fig 1A\n(placeholder)", size = 5) + theme_void()
}
...
fig1_row2 <- fig1b + fig1c + plot_layout(widths = c(1.05, 1))
fig1_row3 <- fig1d
