#!/usr/bin/env python3
"""Class-stratified GO-BP enrichment for LD-mapped class-sum and ratio GWAS genes.
A ratio gene is assigned to both numerator and denominator lipid classes."""
import csv, math, re
from collections import defaultdict
from pathlib import Path
repo=Path('/Users/nirwantandukar/Documents/Github/SAP-Lipidomics-Database')
...
out=repo/'data/LD_mapped/go_enrichment/GO_BP_enrichment_by_lipid_class_sumratio_LD.tsv'
summary=repo/'data/LD_mapped/go_enrichment/GO_BP_enrichment_by_lipid_class_sumratio_LD_summary.tsv'
def parse_terms(v):
    z=[]
    for x in (v or '').split(';'):
        x=x.strip()
...
with open(annot) as fh:
    for r in csv.DictReader(fh,delimiter='\t'): gene_terms[r['GeneID'].strip()].update(parse_terms(r.get('GO_BP','')))
...
for cond in sorted(classgenes):
  for cl,genes in sorted(classgenes[cond].items()):
    test=genes&bg; summ.append({'condition':cond,'lipid_class':cl,'sumratio_phenotypes':len(classph[cond][cl]),'candidate_genes':len(genes),'GO_BP_testable_genes':len(test)})
