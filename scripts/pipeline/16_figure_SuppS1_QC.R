...
create_run_order_plot <- function(raw_file, panel_title) {
  ...
  subtitle <- paste0("QC TIC CV: ", ifelse(is.finite(qc_cv), sprintf("%.1f%%", qc_cv), "not available"))
  ggplot(dat, aes(Run, TIC, colour = Type)) +
    ...
    labs(title = paste0(panel_title, " Run-Order TIC"), subtitle = subtitle, x = "Run order", y = "TIC") + theme_qc
}
...
create_rsd_plot <- function(path, panel_title) {
  ...
  ggplot(d, aes(Stage, RSD, fill = Stage)) +
    ...
    labs(title = panel_title, subtitle = sprintf("QC-RSD <30%%: %.1f%% to %.1f%%", d$Pct[1], d$Pct[2]), x = NULL, y = "Median QC-RSD (%)") +
    expand_limits(y = max(d$RSD) * 1.15) + theme_qc + theme(legend.position = "none")
}

create_pca_plot <- function(input_path, normalized_path, panel_title) {
  ...
  ggplot(d, aes(PC1, PC2, colour = Stage, shape = Type)) +
    ...
    labs(title = panel_title, subtitle = sprintf("Matched injections: %d; matched features: %d", n, length(ids)),
         x = sprintf("PC1 (%.1f%%)", 100 * ve[1]), y = sprintf("PC2 (%.1f%%)", 100 * ve[2])) + theme_qc
}
...
create_residual_plot <- function(condition, panel_title) {
  ...
  ggplot(d, aes(x, y, fill = Residual)) +
    ...
    labs(title = panel_title, subtitle = paste0("MGDG(18:3/18:3); observed minus SpATS fitted value"), x = xlab, y = ylab, fill = "Residual") +
...
p_e <- create_pca_plot(file.path(new_data_dir, "3_CTL_SERRF_input.csv"), file.path(new_data_dir, "SERRF Result_CTL/normalized by - SERRF.csv"), "E")
p_f <- create_pca_plot(file.path(new_data_dir, "4_LIN_SERRF_input.csv"), file.path(new_data_dir, "SERRF Result_LIN/normalized by - SERRF.csv"), "F")
