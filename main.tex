...
FDR $q<0.05$, sized by the number of annotated lipids. Eight of the ten
significant terms are shown; the two that are structural bins rather than
functional or biophysical categories, \textit{PC(38:2)} and \textit{C20:1}, are
reported in Supplementary Table~S5 only. Unlike (A) and (B), this panel is a direct CTL--LIN contrast,
and the two trials differ in year, field and acquisition batch.}
\label{fig:fig2_class_composition}
\end{figure*}

We next asked whether the class shifts were accompanied by shared structural and biophysical properties using LION, a lipid ontology that groups species into such categories \citep{Molenaar2019}. LION ranking analysis across the shared lipid species returned ten significant terms, five higher and five lower with respect to LIN (\(q<0.05\); Fig.~\ref{fig:fig2_class_composition}C; Supplementary Table~S5, LION enrichment). Higher in LIN were \textit{triacylglycerols}, \textit{lipid storage} and \textit{lipid droplet}, all three annotated on the same 44 species, together with \textit{headgroup with neutral charge} (58 species) and \textit{glycerolipids} (61 species). Lower in LIN were \textit{membrane component} (65 species), \textit{low lateral diffusion} (11 species), \textit{lipid-mediated signalling} (23 species), \textit{PC(38:2)} (3 species) and \textit{C20:1} (4 species), the last two resting on too few annotated lipids to interpret individually and reported in Supplementary Table~S5 rather than in the panel. Read by class composition, this is narrower than ten terms imply. The five higher LIN terms describe one pool of the 44 shared TG species plus 12 DG and 5 MG rising from 16.4\% to 18.5\% of annotated signal. Because LION ranks species against each other, the lower LIN terms are the counterpart, not a separate result. \textit{Membrane component} is that counterpart made explicit, with the shared phospholipids falling from 30.2\% to 28.5\% and PC accounting for all of it (24.2\% to 21.9\%). The other two do not follow the composition. DG rises from 12.7\% to 13.8\% and PA, PS, LPC and LPE rise with it, so no signalling pool falls, and the fully saturated phospholipids, the most tightly packed and slowest-diffusing species, take a larger share of the phospholipid pool under LIN rather than a smaller one (13.1\% to 17.6\%). Of the four shifts, it recovers only TG enrichment: galactolipids and the sulfolipid are missing from the ontology, and PS and lysophospholipids have too few shared species for a rank test.
...
Candidate gene counts do not equal signal counts because one significant SNP tags all genes in its LD window. After binning candidates into 250 kb windows, there were 358 distinct windows under CTL and 949 under LIN. So, LIN captures $\sim$2.6$\times$ more loci, not the four-fold suggested by gene counts. Hence, CTL loci average 3.0 candidate genes, while LIN loci average 4.6. Candidates also differ in phenotype recurrence. Under CTL, 586 of 1,062 individual-lipid candidates were linked to more than one phenotype (median 2, maximum 13), against 1,474 of 4,370 under LIN (median 1, maximum 8). Thus, in the individual-lipid layer CTL yields fewer loci with many lipid associations, while LIN yields more loci with few. The class-sum and ratio layer is too small under CTL for the same contrast, with 27 of its 54 candidates linked to more than one phenotype (median 1.5, maximum 6) against 84 of 385 under LIN (median 1, maximum 13). Because lipid phenotypes are inherently highly correlated by design, these recurrence counts serve as descriptive summaries rather than formal tests against a null hypothesis; the cross-trial comparison presented in the next section is evaluated explicitly.

In the CTL individual-lipid layer, the strongest association and the most recurrent locus were the same gene, \texttt{SORBI\_3005G148500} (FBD domain-containing protein; 13 phenotypes; $p = 1.55\times10^{-16}$; $r^2_{\max}=0.52$). In the CTL sum/ratio layer, the strongest association was \texttt{SORBI\_3006G010000}, an unannotated candidate gene ($p = 2.02\times10^{-10}$; $r^2_{\max}=0.67$), and the most recurrent locus was \texttt{SORBI\_3001G258000} (protein kinase domain-containing protein; 6 phenotypes; $p = 3.17\times10^{-9}$; $r^2_{\max}=0.66$) (Table~\ref{tab:top_loci}). In the LIN individual-lipid layer, the strongest association was observed for \texttt{SORBI\_3008G081000}, also unannotated ($p = 4.50\times10^{-33}$; $r^2_{\max}=0.94$), and the most recurrent locus was \texttt{SORBI\_3010G150600} (8 phenotypes; $p = 1.41\times10^{-18}$; $r^2_{\max}=1.0$), one of nine genes in a chromosome~10 interval labelled by a peroxidase. In the LIN sum/ratio layer, the strongest association and the most recurrent locus were both \texttt{SORBI\_3004G004800} (soluble acid invertase; 13 phenotypes; $p = 7.07\times10^{-16}$; $r^2_{\max}=0.69$) (Table~\ref{tab:top_loci}).
...
Sums / ratios & 1:30.37--30.84 & 9 & \texttt{SORBI\_3001G257300} & F-box domain-containing protein & 6 & $3.17\times10^{-9}$ \\
 & 6:1.45--1.57 & 6 & \texttt{SORBI\_3006G010100} & Terpene synthase & 3 & $2.02\times10^{-10}$ \\
 & 1:30.70--31.02 & 5 & \texttt{SORBI\_3001G258600} & GDSL esterase/lipase & 3 & $5.80\times10^{-9}$ \\
...
Sums / ratios & 4:0.42--0.78 & 27 & \texttt{SORBI\_3004G004800} & Soluble acid invertase & 13 & $7.07\times10^{-16}$ \\
 ...
 & 4:0.18--0.43 & 3 & \texttt{SORBI\_3004G005000} & RING-type domain-containing protein & 10 & $1.48\times10^{-14}$ \\
...
Cross-referencing branches with the LIN GWAS candidate tables supported three of four (Supplementary Tables~S6 and S9). Branch~(i) was supported by two LCAT-superfamily genes, \texttt{SORBI\_3006G214500} and \texttt{SORBI\_3004G341900}, whose closest Arabidopsis homologues are LCAT4 and PSAT; characterized members of this family release a lysophospholipid from PC or PE rather than transferring the acyl group to diacylglycerol \citep{Chen2012LCATPLA, Banas2005PSAT}; branch~(ii) by \texttt{SORBI\_3001G041900}, a PNPLA-domain lipase in the SDP1 clade associated with three TG species under LIN; and branch~(iv) by \texttt{SORBI\_3010G170000} (\textit{DGAT1}). No candidate was found for branch~(iii), and no PDAT-family gene was recovered in either trial (Supplementary Table~S9, branch summary). Together, these results link the class-level composition shift to four candidate reactions, three also showing a LIN genome-wide association.
