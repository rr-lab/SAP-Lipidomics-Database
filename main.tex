...
SAP is structured by botanical race and geographic origin \citep{Casa2008CommunityResourcesSorghum, Boatwright2022}. Next, we tested whether lipid variation is affected by race or marker-based genetic clusters ($K=6$). Using Kruskal–Wallis with BH correction within trials (Methods; Supplementary Table~S1), we found little association. No lipid-class sum was significant for race in either trial (smallest $q=0.468$, $\varepsilon^2\le0.027$). The genetic cluster was significant only for PS under LIN ($q=1.11\times10^{-6}$, $\varepsilon^2=0.104$; Fig.~\ref{fig:fig1_population_structure}), because PS contains a single LIN species, PS(18:0/18:2). This reflects the species which showed only a weak CTL trend ($\varepsilon^2=0.020$, $q=0.358$). Furthermore, none of the species-level tests were significant under CTL, and five under LIN, all by genetic cluster, led by PS(18:0/18:2) ($\varepsilon^2=0.088$, $q=2.0\times10^{-4}$; Table~\ref{tab:species_ancestry}; Supplementary Table~S2 (per-species structure and $h^2$)). PCA of class composition likewise showed minimal structure, with groupings explaining $\le$1.6\% of variation (Supplementary~\ref{fig:suppfig_s3_class_pca}). Thus, ancestry is not a confounder.

Subsequently, we also quantified genetic variance in the lipidome by estimating genomic heritability ($h^2$) for 146 species detected in both trials across the same 351 genotypes (Supplementary Table~S2, per-species paired). Under CTL, median $h^2$ was near zero and 11/146 species had 95\% profile-likelihood intervals excluding zero. In contrast, under LIN, the median $h^2$ was 0.171 and 45/146 excluded zero. Species-wise, $h^2$ was higher under LIN for 118 species, higher under CTL for 19, and equal for 9 (sign test $p = 1.2\times10^{-18}$; Supplementary~\ref{fig:suppfig_s4_heritability}). The class sums showed the same pattern, with all classes except TG and PA higher under LIN (Supplementary Table~S2, class sums). This difference was robust to population structure and sampling. Moreover, adding the top three relationship-matrix eigenvectors as fixed effects left 43/45 LIN-heritable species excluding zero (Supplementary Table~S2, structure-conditioned), and removing any race group or 20\% of accessions kept CTL and LIN counts non-overlapping (20--26 vs 37--63; Supplementary Table~S3, composition stability, and Supplementary Table~S2, ancestry robustness). 
...
We quantified a total of 238 lipid species, with 146 detected in both trials, 48 only in CTL, and 44 only in LIN, where “trial-specific” indicates detection above threshold in one trial rather than absence in the other (Supplementary~\ref{fig:suppfig_s5_counts}). TGs and PCs were the most diverse classes in both environments (TG: 67 in CTL, 66 in LIN; PC: 36 in CTL, 38 in LIN; Supplementary~\ref{fig:suppfig_s5_counts}). Per-species detection was stable to genotype jackknifing, both overall and within each class (Supplementary Table~S3, jackknife and stability by class), and the species contributing most to between-genotype variance in each trial are listed in Supplementary Table~S4 (top-variance lipids).
...
each, is given in Supplementary Table~S7 (loci collapsed).}
