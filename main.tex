...
In total, 433 of 1{,}623 tested terms passed the LD-aware null ($q_{\mathrm{LD}}<0.05$). Because GO terms are nested and class sums and ratios share candidate genes, we merged terms with identical gene sets, leaving 285 term--gene-set combinations across 189 distinct terms (Supplementary Table~S16, loci collapsed). Of these, 102 draw on $\leq 30$ background genes; the rest were too broad to interpret (\textit{zinc ion binding}, 909 genes; \textit{transmembrane transport}, 865) and pointed to no lipid-specific process. Eighty-five of the 102 are supported by two or more independent intervals, and the remaining 17 rest on a single LD block. Figure~\ref{fig:go_bp_main} shows the strongest four per lipid class among the 85, giving 23 gene sets.

Those 23 gene sets are unevenly split between trials, with 19 under LIN and 4 under CTL, the same asymmetry that the candidate counts show. All four CTL gene sets come from TG traits. The LIN gene sets centre on nutrient transport and signaling. \textit{Cellular response to phosphate starvation} is the strongest of them, at $51\times$ enrichment across three independent intervals, and is the only gene set drawn from two lipid classes at once, fatty acids and SQDG. \textit{Nitrate transmembrane transporter activity} follows from fatty-acid traits at $25\times$ across two intervals, and \textit{sphingolipid metabolic process} from AEG at $35\times$. Fatty acids, PC, SQDG and TG each contribute four gene sets. Enrichment therefore narrows five thousand candidate genes to a short list which could be used for functional validation.
