...
Subsequently, we quantified genetic variance in the lipidome by estimating genomic heritability ($h^2$) for every species detected in each trial (214 under CTL, 216 under LIN) and compared the trials on the 164 species detected in both and measured across the same 351 genotypes (Supplementary Table~S2, per-species paired). Under CTL, the median $h^2$ was near zero, and only 11 of those species had heritability that we could distinguish from zero. Under LIN, the median was 0.171, and 54 species did. The same direction held for species by species comparison, with $h^2$ higher under LIN for 132 species, higher under CTL for 21, and equal for 11 (Supplementary~\ref{fig:suppfig_s4_heritability}), and for class by class comparison where every class was higher under LIN except TG and PA (Supplementary Table~S2, structure-conditioned). Estimating $h^2$ directly on the 17 class sums, using every accession and every species of each trial (389 accessions under CTL, 357 under LIN), gives the same picture. The median was 0 under CTL and 0.117 under LIN, 15 of the 17 classes were higher under LIN (paired Wilcoxon $p=0.007$), and the two exceptions were again TG and PA. Four classes had an interval excluding zero in one trial or the other (Table~\ref{tab:classsum_h2}; Supplementary Table~S2, class sums), so most class sums carry no heritability that can be distinguished from zero, and the trial difference is a shift in the whole distribution rather than a handful of strongly heritable classes. Two checks indicate that this is not an artifact of ancestry or of which genotypes were sampled. Fitting the three leading eigenvectors of the relationship matrix as fixed effects, which removes variation between genetic clusters, left 51 of the 66 LIN-heritable species still heritable (Supplementary Table~S2, structure-conditioned). Re-estimating after dropping each botanical race in turn and after twenty random 80\% subsamples of the panel gave 14 to 26 heritable species under CTL and 43 to 87 under LIN on the full per-trial sets. The two ranges never overlap, so LIN exceeded CTL in every run (Supplementary Table~S2, ancestry robustness). 
...
All 17 are in Supplementary Table~S2, class sums. $N$ is the number of species summed into the class in that trial.}
...
supporting each, is given in Supplementary Table~S7 (loci collapsed).}
...
In total, 344 of 1{,}292 tested terms passed the LD-aware null ($q_{\mathrm{LD}}<0.05$). Because GO terms are nested and class sums and ratios share candidate genes, we merged terms with identical gene sets, leaving 217 term--gene-set combinations across 159 distinct terms (Supplementary Table~S7, loci collapsed). Of these, 87 draw on $\leq 30$ background genes; the rest were too broad to interpret (\textit{zinc ion binding}, 909 genes; \textit{transmembrane transport}, 865) and pointed to no lipid-specific process. Seventy-one of the 87 are supported by two or more independent intervals, and the remaining 16 rest on a single LD block. Figure~\ref{fig:go_bp_main} shows the strongest four per lipid class among the 71, giving 22 gene sets.

Those 22 gene sets are unevenly split between trials, with 18 under LIN and 4 under CTL, the same asymmetry that the candidate counts show. All four CTL gene sets come from TG traits. The LIN gene sets centre on nutrient transport and signaling. \textit{Cellular response to phosphate starvation} is the strongest of them, at $51\times$ enrichment across two independent intervals, and is one of only two gene sets in the figure drawn from more than one lipid class, here fatty acids and SQDG. \textit{Nitrate transmembrane transporter activity} follows from fatty-acid traits at $25\times$ across two intervals, and \textit{sphingolipid metabolic process} from AEG at $35\times$. Fatty acids, PC and TG each contribute four gene sets, and SQDG three. Enrichment therefore narrows five thousand candidate genes to a short list which could be used for functional validation.
...
Intervals are 95\% profile-likelihood intervals, defined as the set of \(h^2\) values on a grid whose restricted log-likelihood falls within \(\chi^2_1/2\) of the maximum. A trait was called non-zero when this interval excluded zero. Bootstrap resampling was not used, as resampling genotypes with replacement creates pairs with relationship 1 and identical phenotypes, which forces the estimate to 1 in every replicate. For the per-species comparison, CTL and LIN were fitted using the same 351 genotypes and the same 164 species present in both trials, so the trials differed only in the phenotype. The class sums were instead fitted on every accession and every species of each trial, on the summed spatially corrected intensity of the class transformed as \(\log_{10}(x+1)\), the same construction used for the per-species estimates and the usual scale for mass-spectrometry abundances. Both trials were also analysed jointly. Values were standardised within trial, and each genotype's mean across trials and difference between trials were fitted with the same model and the same interval, so that the heritability of the mean estimates the genotype main effect and the heritability of the difference estimates genotype-by-environment variance. Because the trials differ in year, field, planting date and analytical batch and neither is replicated, the environment term is the whole trial contrast rather than the nutrient treatment.
...
same 351 genotypes and the same 164 species detected in both trials, so there
