...
The interface allows for easy tracing of traits to candidate genes through the combination of filtering, plotting, and gene summaries. In the \emph{Gene Hits} module (LIN dataset; all trait sources; genome-wide threshold; strict LD, $r^2 \geq 0.9$), 1,325 candidate genes are output and ranked by the number of phenotypes in which they recur (Fig.~\ref{fig:Fig5_shiny_app}B). The top gene is a chromosome 4 cytochrome P450 (\texttt{SORBI\_3004G006400}), recurring in 21 phenotypes. Next are five adjacent chromosome 3 genes. Each carries a variant in perfect linkage disequilibrium ($r^2 = 1$) with the most significant SNP at the locus, recurs in 19--20 phenotypes, and shares the same association ($p = 4.46 \times 10^{-20}$). These relate to one class sum (free fatty acids), one species (FA(22:1)), and 18 ratios. In the 500~kb window (7.40--7.90~Mb) around these genes, genome-wide significant SNPs and high-LD variants are restricted to $\sim$7.70--7.78~Mb (Fig.~\ref{fig:Fig5_shiny_app}C), and 28 of the 49 genes in the window were not selected ($r^2 < 0.4$). Three of the five block genes carry annotation, and they split into two kinds. \texttt{SORBI\_3003G088800} and \texttt{SORBI\_3003G088900} are near-identical neighbors, closest to rice \textit{OsLPR2} and \textit{OsLPR1} (76\% and 75\% amino-acid identity), which encode multicopper oxidases of the LOW PHOSPHATE ROOT family that govern the root meristem response to phosphate deficiency. \texttt{SORBI\_3003G089000}, encoding a ubiquitin-conjugating E2 whose closest annotated Arabidopsis relative is UBC24/PHO2 which acts as a negative regulator that limits phosphate uptake and root-to-shoot translocation. Two arms of the phosphate-starvation response, sensing at the root and restraint of transport, therefore fall within a single LD block detected under LIN. LD cannot distinguish among the five genes, and the associations are with fatty-acid ratios rather than phospholipid abundances. The SoLD Shiny app brings together lipid phenotypes, their associations, and LD-supported candidate genes, enabling direct recovery and inspection of loci (e.g., the chromosome 3 block), comparison of compositional and multivariate views across two trials, and selection of accessions by lipid content for follow-up experiments.

\section{Discussion}

Lipidome differences between CTL and LIN were population-wide rather than driven by a few outlier genotypes. Lipid-trait loci, in contrast, were largely distinct between the two trials. We first link the lipidome changes to trial conditions, then discuss candidate genes, and finish with the near-absence of shared loci between trials, which points to different genes acting on the same pathways in each environment. Candidates were identified through two routes. Most fall within ontology terms passing the LD-aware permutation null, supported by both enrichment and association (Supplementary Table S19). A second set comes from the LINEX reaction network, retaining genes mapped to branches flagged as remodeled between trials. However, two caveats apply. First, gene functions are inferred by orthology to characterized genes in other species. Second, candidates come from LD blocks, so most associated regions contain several genes which should also be taken into account for validation.

\subsection*{Integrated interpretation of lipidome remodeling across contrasting field trials}

The two most prominent changes in LIN are a decrease in SQDG and an increase in TG. The decline in SQDG contradicts the expected phosphate-starvation response, typically characterized by a rise in SQDG and DGDG as phospholipids are replaced with non-phosphorus glycolipids \citep{Essigmann1998, Hartel2000, Jouhet2004, KellyDormann2002, Pfaff2020}. Instead, reduced SQDG suggests influences from various factors such as nutrient limitation, temperature changes, and developmental stage. The LIN GWAS set included two SQD2-like sulfoquinovosyltransferase loci (\texttt{SORBI\_3003G076900}, \texttt{SORBI\_3002G000600}) linked to sulfolipid biosynthesis. However, they were candidates from fatty-acid phenotypes rather than SQDG levels, making them potential candidates for further investigation. The accumulation of TG is commonly associated with low temperatures, drought, oxidative stress, and nutrient-driven carbon reallocation, suggesting a shift towards increased storage within the LIN signature \citep{Tan2018, Yan2025, Pfaff2020}. The median LIN balances are higher than CTL for TG/DG (2.2×), TG/MGDG (2.7×), TG/SQDG (3.6×), and TG/PE (1.3×). The exception is TG/MG (0.74×), reflecting a faster rise in MG. DG shifts oppositely as DG/MG is 0.34× and DG/PS is 0.13×. Correlations also change. PS–TG switches from negative in CTL ($r=-0.26$) to positive in LIN ($r=+0.43$), a strong sign reversal (Supplementary Table S5C). Additionally, LINEX reactions add to these results. TG-producing branches shift toward products under LIN, while TG→DG hydrolysis shifts toward substrate, collectively favoring TG over DG (Supplementary Table S12). These class-level balances together indicate a reorganization of DG supply and TG synthesis and mobilization, though direct measures of enzyme activity are needed.

Another change involves the balance among phospholipid headgroups. PS increased against every class it was tested against (Supplementary Table S1), and the PS--PC correlation flipped from positive in CTL ($r=+0.45$) to negative in LIN ($r=-0.34$), the largest shift among class pairs (Supplementary Table S5C). PE showed the largest increase in mean double-bond number of any class, although the species detected differs slightly between trials, and this is a composition measure rather than a direct account of acyl editing. Comparable shifts in headgroup proportions, membrane unsaturation, and lysophospholipid pools are reported under cold, nutrient, oxidative, and water limitation, where they accompany changes in membrane fluidity, lipid repair, signaling, and remobilization \citep{Hou2016, Rawat2021, Yan2025, Zhang2020}. The balance between the lysophospholipid classes also shifted, with LPC/LPE dropping to 0.38-fold in LIN (Supplementary Table S1), driven by LPE rising from 0.0065\% to 0.0139\% of total annotated signal while LPC was essentially unchanged (Supplementary Table S5D). PS and the lysophospholipids each contribute under 0.2\% of TIC, so these are large proportional changes on small pools. The annotation is also shallow, with four LPC species under CTL and three under LIN and a single LPE species in both, so the shift cannot be read as evidence about phospholipase activity.

These compositional changes coincide with a shift in how strongly lipid variation reflects genotype. Under CTL, median $h^2$ was near zero (11/146 species excluded zero), while, under LIN, median $h^2$ was 0.171 (45/146 excluded zero), with $h^2$ higher under LIN for 118 species versus 19 under CTL. This difference persisted after conditioning on population structure and after removing any race group or 20\% of accessions. It is driven by increased genetic variance under LIN. Total variance was similar across trials, and residual variance was lower under LIN in only about half of the species, whereas the genetic term rose from a median of zero and was larger under LIN in about 90\% of species. A similar one-sided pattern holds for ancestry, though effects are small. No lipid-class sum was associated with botanical race in either trial (smallest $q=0.468$, $\varepsilon^2 \leq 0.027$). No species was significant under CTL, whereas five were under LIN. All by marker-based genetic cluster (not race), led by PS(18:0/18:2) ($\varepsilon^2=0.088$, $q=2.0\times10^{-4}$). Thus, ancestry explains little in either trial, and a detectable signal appears only under LIN. Moreover, the GWAS mapping results show the same asymmetry, with traits that have higher heritability being simpler to map, so the longer LIN candidate list likely indicates higher $h^2$ rather than a greater number of genes. Candidate loci were largely trial-specific. Gene-level overlap exceeded chance ($2.39\times$) but largely reflected LD, losing significance after collapsing to 250 kb loci. Only 90/354 shared genes (25.4\%) mapped to a common lipid class. Class-level composites were the exception, remaining enriched after collapsing, making them the most reproducible unit across trials. Because the trials differ by year, field, and batch, as well as nutrients, these results do not isolate a genotype-by-nutrient effect; testing that would require repeating the nutrient contrast within a single year.
...
Under LIN, no tyrosine or dhurrin term is enriched. Instead, dhurrin is implicated by candidate annotations at a single chromosome 1 locus. Seven candidates share 12 of 13 PA-to-class ratios, three of them also carry the fatty-acid class sum and FA(22:1), and four carry TG(14:0\_18:3\_18:3). Three are known dhurrin-cluster genes \citep{Darbani2016Dhurrin}: UGT85B1 (\texttt{SORBI\_3001G012400}), SbMATE2 (\texttt{SORBI\_3001G012600}), and SbGST1 (\texttt{SORBI\_3001G012500}); a fourth, \texttt{SORBI\_3001G012000}, is a CYP71 P450 annotated to the oxime monooxygenase step. The cluster P450s CYP71E1 (\texttt{SORBI\_3001G012200}) and CYP79A1 (\texttt{SORBI\_3001G012300}) are not candidates. Only two annotated candidates enter any enriched term, both broad (iron ion binding for the P450; transmembrane transporter activity for the transporter), whereas UGT85B1 and SbGST1 enter none. Thus, the pathway signal comes from co-localized cluster members and their annotations, not enrichment.
...
Nitrogen transport enters the LIN candidate set through two GO enrichments. The first is specific but narrow. \textit{Nitrate transmembrane transporter activity}, \textit{nitrate assimilation} and \textit{nitrate transmembrane transport} are enriched in the FA and SQDG sets each rests on a single 250~kb interval on chromosome~4. The second is broad but distributed \textit{Transmembrane transporter activity} and \textit{transmembrane transport}.

The nitrate terms are driven by \texttt{SORBI\_3004G009400} (NRT2.1-like, high-affinity) together with two adjacent genes of the same family, \texttt{SORBI\_3004G009200} and \texttt{SORBI\_3004G009500}, all three within a 250~kb interval on chromosome~4. \texttt{SORBI\_3004G009400} is the best supported of the three, and is associated with FA(22:1) and with the FA/SQDG ratios. That interval tags 23 LIN candidate genes in total, so LD does not single out the transporters within it. In maize, NRT2.1 requires NAR2.1, and its capacity varies along the root with plasma-membrane H$^+$-ATPase activity \citep{Lupini2016NAR2, Sorgona2011Nitrate}.

Two additional NPF/NRT1 transporters, \texttt{SORBI\_3008G081400} and \texttt{SORBI\_3001G133900}, appear in the broad transport terms. \texttt{SORBI\_3008G081400} is recovered in both trials (seven palmitate-containing TGs under CTL; FA(22:1) and TG(18:1\_18:2\_18:2) under LIN). Its closest Arabidopsis homolog is NPF4.6/NRT1.2/AIT1, which transports both nitrate and abscisic acid without the two competing as substrates \citep{Kanno2013NPF46}. The same protein interacts with PHOSPHOLIPASE D$\alpha$1 at the plasma membrane, and \textit{PLD$\alpha$1} is epistatic to \textit{NRT1.2} in ABA responses \citep{Li2020NRT12PLD}. The homolog therefore sits at a junction of nitrate transport, ABA transport and phospholipase D activity, which is a plausible point of contact between nitrogen status and membrane lipid turnover, although the PLD$\alpha$1 interaction was characterized in seed germination rather than in leaves. \texttt{SORBI\_3001G133900} associates with one lipid, TG(14:0\_18:2\_18:3), and its rice ortholog OsNPF2.4 mediates low-affinity nitrate root-to-shoot transfer \citep{Xia2015NPF24}. Thus, these three transporters are the most promising nitrogen-related candidates in the LIN set for follow-up work.

\subsection*{Phosphate and sulfur candidates recovered under LIN}

Phosphorus and sulfur enter via two SQDG-linked loci. Under LIN, the term cellular response to phosphate starvation is enriched in the SQDG class-sum and ratio, and its three genes are \texttt{SORBI\_3001G028500} on chromosome 1 and \texttt{SORBI\_3003G088800} and \texttt{SORBI\_3003G088900} on chromosome 3. \texttt{SORBI\_3001G028500} encodes an SPX-domain phosphate-status signaling protein, and like rice OsSPX4, which negatively regulates phosphate signaling through its interaction with PHR2, it may regulate phosphate-starvation responses \citep{Lv2014SPX4}. The two chromosome 3 genes are adjacent and closest to rice \textit{OsLPR2} and \textit{OsLPR1}, the multicopper oxidases of the LOW PHOSPHATE ROOT family, which govern the root meristem response to phosphate deficiency and are situated as a tandem array of five genes within 65 kb of rice chromosome~1 \citep{Cao2016OsLPR}. \texttt{SORBI\_3006G244000} encodes a SULTR3-family sulfate transporter. In Arabidopsis, SULTR3 proteins localize to the chloroplast envelope and mediate chloroplast sulfate uptake, with strong effects on sulfate import \citep{Chen2019SULTR3}. Because SQDG is a plastid lipid whose sulfur-containing head group depends on plastid sulfate import and reduction, SULTR3 lies on the sulfur supply route for SQDG. Phosphate limitation typically increases SQDG by replacing phospholipids. Yet, SQDG was depleted under LIN, making limited plastid sulfate supply a plausible follow-up hypothesis rather than a demonstrated mechanism.

A phosphate transporter gene was also identified. \texttt{SORBI\_3002G116100} encodes a PHT1-family H$^+$/Pi cotransporter and was a LIN candidate for five species, mainly TGs (including TG(18:3\_18:3\_18:3)). It is included in the broad \textit{transmembrane transporter activity} term (not shown in Fig.~\ref{fig:go_bp_main}). Its closest characterized rice relative is OsPT4 (\textit{OsPht1;4}), which affects phosphate accumulation and embryo phosphate mobilization \citep{Ye2015OsPT4,Zhang2015OsPht14}. Interpretation of this locus should be cautious, as it is the weakest association among the loci discussed here and only marginally exceeds the Bonferroni threshold. The associated traits are also neutral storage lipids rather than phospholipids, which do not fit a simple phospholipid-to-glycolipid replacement model under phosphate limitation.

The strongest LIN panel signal implicates the same phosphate-response pathway. The chromosome 3 block contains two LOW PHOSPHATE ROOT (LPR) homologues, \texttt{SORBI\_3003G088800} and \texttt{SORBI\_3003G088900}, adjacent to a PHO2-family ubiquitin-conjugating E2, \texttt{SORBI\_3003G089000}. In Arabidopsis, \textit{LPR1/2} encode multicopper oxidases that control root meristem responses to phosphate deficiency. Additionally, in rice, the family has expanded to five genes, \textit{OsLPR1}–5, within 65 kb of chromosome 1, of which \textit{OsLPR3} and \textit{OsLPR5} are induced by Pi deficiency \citep{Cao2016OsLPR}. \texttt{SORBI\_3003G088800} and \texttt{SORBI\_3003G088900} are likewise adjacent, separated by 1.8 kb, although their closest rice relatives are \textit{OsLPR2} and \textit{OsLPR1} rather than the Pi-induced pair. PHO2/UBC24 is a negative regulator that limits phosphate uptake and root-to-shoot transport. \textit{pho2} overaccumulates Pi due to a nonsense mutation in \textit{UBC24}, and \textit{PHO2} is miR399-cleaved under Pi deficiency \citep{Aung2006PHO2}. PHT1 transporters and PHOSPHATE TRANSPORTER TRAFFIC FACILITATOR1 act downstream of PHO2 \citep{Huang2013PHO2}, and in Arabidopsis the RING-type E3 ligase NITROGEN LIMITATION ADAPTATION (NLA) partners with UBC24 to ubiquitinate PT2 \citep{Park2014NLA}. This NLA link aligns with our design because LIN reduced both N and P. The block contains five genes in total, including a ubiquitin-conjugating E2 (\texttt{SORBI\_3003G089000}), a WD40-repeat protein (\texttt{SORBI\_3003G089100}), and a glutamyl-tRNA amidotransferase subunit (\texttt{SORBI\_3003G089200}), none of which belong to an enriched term. Thus, these loci are prioritized for functional validation rather than being offered as known mechanisms, and separating the contributions of the genes within each block will require targeted assays in contrasting accessions.

\subsection*{A TIFY/JAZ region enriched under LIN is also recovered under CTL for different lipids}

Under LIN, three TIFY-domain genes on chromosome 1 (\texttt{SORBI\_3001G259300}, \texttt{SORBI\_3001G259400}, \texttt{SORBI\_3001G259600}) drive the enrichment of the regulation of defense response, regulation of the jasmonic acid mediated signaling pathway, and response to wounding, all from the AEG trait class. All three associate with AEG(o-15:2\_15:0) and PC(14:0\_18:3), and \texttt{SORBI\_3001G259400} and \texttt{SORBI\_3001G259600} additionally associate with 11,14,17-eicosatrienoic acid. The same region is also recovered under CTL, but for different lipids and without a corresponding enrichment. There, \texttt{SORBI\_3001G259400} and \texttt{SORBI\_3001G259600} are candidates for the nine TG species, seven of which contain 16:0. \texttt{SORBI\_3001G259700} is a candidate for four of those nine, and none of the genes is a candidate for any TG under LIN.
...
Developmental timing and cold response converge on \texttt{SORBI\_3003G406800}, a MADS-box transcription factor associated with fatty-acid traits under LIN for FA(22:1), PC(18:2\_20:4), the fatty-acid class sum, and ten FA-to-class ratios. It carries the term \textit{positive regulation of transcription by RNA polymerase II} in the SQDG class-sum and ratio layer.

Multiple studies support a cold-response role. In sorghum, this gene is induced within 6 h of cold, and it is a hub in the early shock co-expression module ($k_{\mathrm{ME}}=0.91$) \citep{Kurt2026ColdSorghum}. In maize, \textit{ZmMADS69}, a flowering-time locus, regulates ZmRap2.7/\textit{ZCN8} and was selected during tropical-to-temperate adaptation \citep{Liang2019ZmMADS69}.

LIN was planted earlier to expose the plants to cold during establishment, so a cold-induced gene linked to fatty-acid traits fits the treatment. Cold acclimation also involves acyl-chain remodeling to maintain membrane fluidity \citep{Tan2018, Zhang2020}. However, LIN also differs from CTL in nitrogen, phosphorus, and year, so temperature cannot be isolated. A MADS-box flowering-time regulator may instead reflect a developmental-stage shift at sampling rather than lipid regulation, which this design cannot disentangle. Earlier planting also changes the photoperiod and temperature a plant experiences at each stage, so a flowering-time locus would be expected to differ between the trials, whether or not it acts on lipids. Thus, this locus is a strong candidate for earlier-planting, with cold response and developmental timing.

\subsection*{Reaction-supported lipid-remodeling candidates under LIN implicate DG--TG cycling and phospholipid turnover}

Two candidates feed the DG node from different directions. \texttt{SORBI\_3007G223801}, an NAD-dependent glycerol-3-phosphate dehydrogenase, supplies the glycerol backbone for glycerolipids. Overexpression of the Arabidopsis \textit{SFD1}/\textit{GLY1} orthologue increases plastid membrane lipids in rice \citep{Singh2016SFD1}. It is a LIN candidate for class-ratio traits, including 17 diacylglycerol ratios, although the lead SNP tags five genes. Phospholipid input is represented by \texttt{SORBI\_3003G107900}, a non-specific phospholipase~C6 (NPC). NPCs convert phosphatidylcholine to diacylglycerol and phosphocholine, and this locus associates with PC(18:2\_20:4), a plausible substrate \citep{Bose2021NPC, Ngo2019NPC}. The gene lies in a 137 kb region on chromosome 3 containing five EXPERA-domain sterol C-8 isomerase genes \citep{Souter2002HYD1}, and all six share the association at $r^2 \geq 0.95$, so the phospholipase is put forward based on its annotation rather than because LD singles it out. That block captures five of the six annotated C-8 sterol isomerase genes in the genome (Supplementary Table~S16).

The same LD ambiguity affects branch~(i), the proposed PC$+$DG$\leftrightarrow$LPC$+$TG transfer, which has two LCAT-superfamily candidates. \texttt{SORBI\_3006G214500} links to DG(18:2\_18:3) and MG(18:2), whereas \texttt{SORBI\_3004G341900} is annotated as a phospholipid--sterol O-acyltransferase, supporting an alternative reaction in which sterol esterification using PC as the acyl donor releases LPC without producing TG. This fits the rise in $\beta$-sitosterol under LIN, which increases by more than fourfold. However, both assignments are uncertain, since an Arabidopsis LCAT-family protein acts as a PC-specific phospholipase~A rather than a transferase \citep{Chen2012}. The association is therefore consistent with phospholipid deacylation and LPC production but does not identify the acyl acceptor.

Both arms of DG--TG cycling are captured. \textit{DGAT1} (\texttt{SORBI\_3010G170000}) is a LIN candidate for six phenotypes, five of which are polyunsaturated TGs, including TG(18:3\_18:3\_18:3), with the lead SNP at $r^2 = 1$ for three of them, within a chromosome 10 crude-fat QTL that has been repeatedly mapped in sorghum \citep{Boyles2017DGAT1, Yen2008DGAT}. The reverse arm, \texttt{SORBI\_3001G041900}, associates with three TG species and, at the class level, with total TG and TG/SQDG, consistent with a bulk TG lipase. In Arabidopsis, \textit{SDP1} and \textit{SDP1L} drive most TG breakdown during reserve mobilization \citep{Kelly2011SDP1L}, and this locus groups with \textit{SbSDP1-L} in the sorghum lipase phylogeny \citep{Sapara2024PLA}.

A second module lies outside these branches. The strongest lipid-pathway signal was \textit{sphingolipid metabolic process}, supported by three independent intervals and three genes spanning three steps of the pathway on three chromosomes, a serine C-palmitoyltransferase (\texttt{SORBI\_3001G434900}), a DAGK-domain kinase (\texttt{SORBI\_3007G041500}), and a ceramide glucosyltransferase (\texttt{SORBI\_3003G394100}). All associate with AEG(o-15:2\_15:0), the ether-linked species with much the largest increase under LIN. This evidence is genetic rather than compositional because sphingolipid levels change little and these species are trace.

Together, these loci point to backbone supply, phospholipid head-group hydrolysis, and both arms of DG–TG cycling as distinct modules needing functional tests in sorghum. Prioritizing candidates by reaction distinguishes enzymes within broad terms like \textit{triglyceride metabolic process} (e.g., synthase vs lipase). Thus, reaction mapping offers an additional, enrichment-complementary way to prioritize genes linked to LIN lipid changes.
