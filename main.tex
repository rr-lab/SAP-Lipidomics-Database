...
Compared with CTL, four population-wide shifts characterize the LIN lipidome: (i) SQDG depletion, (ii) TG enrichment, (iii) a phospholipid rebalancing centered on PS, and (iv) an LPC/LPE balance shifted toward LPE. All four rest on the abundance and composition analyses. LION corroborates only the TG shift, its remaining terms tracking the same glycerolipid-versus-membrane contrast. The chemical-space summary adds no further shift, because within-class chain composition is largely stable and the CTL--LIN difference sits in class proportions rather than chain length or unsaturation. Because the trials also differ by year, field, planting date, and analytical batch, these shifts characterize LIN as a whole and cannot be attributed to just nutrient supply and cold. 

The reaction-network and GWAS sections that follow ask which reactions and which loci accompany them. Candidate genes follow for the TG and SQDG shifts. The PS and lysophospholipid changes sit on pools under 0.2\% of TIC and are not resolved to candidates, although PS is the one class whose abundance tracks genetic cluster (Fig.~\ref{fig:fig1_population_structure}C,D).


\subsection*{GWAS recovers more genomic regions under LIN and more trait-sharing per region under CTL}

We ran separate GWAS for each individual lipid trait, lipid-class sum, and lipid-class ratio in both trials (1,102 in total: 225 individual species and 324 class sums and ratios under CTL, 229 and 324 under LIN), using a univariate linear mixed model with the first three genotype PCs to account for population structure (Fig.~\ref{fig:gwas_manhattan}). SNPs surpassing the Bonferroni genome-wide significance threshold ($p \leq 8.19 \times 10^{-9}$ for CTL and $8.22 \times 10^{-9}$ for LIN) were mapped to candidate genes using the LD-based cutoff of 0.4. Under CTL, this identified 1,100 candidate genes for individual lipids and 115 for class sums and ratios (Supplementary Table~S6). Under LIN, the same procedure identified 4,319 and 812 candidate genes (Supplementary Table~S6). Manhattan and QQ plots for every lipid phenotype in both trials are available in the SoLD Shiny application.


Candidate gene counts do not equal signal counts because one significant SNP tags all genes in its LD window. After binning candidates into 250 kb windows, there were 378 distinct windows under CTL and 970 under LIN. So, LIN captures $\sim$2.6$\times$ more loci, not the nearly four-fold suggested by gene counts. Hence, CTL loci average 2.9 candidate genes, while LIN loci average 4.5. Candidates also differ in phenotype recurrence. Under CTL, 612 of 1,100 individual-lipid candidates were linked to more than one phenotype (median 2, maximum 25), against 1,302 of 4,319 under LIN (median 1, maximum 12). The class-sum/ratio layer shows the same pattern (CTL median 3, maximum 64; LIN median 1, maximum 20). Thus, CTL yields fewer loci with many lipid associations, while LIN yields more loci with few. Because lipid phenotypes are inherently highly correlated by design, these recurrence counts serve as descriptive summaries rather than formal tests against a null hypothesis; the cross-trial comparison presented in the next section is evaluated explicitly.
...
Table~\ref{tab:top_loci} lists the five most recurrent loci in each trial and trait layer, counted per signal rather than per gene, with the number of genes each signal tags alongside it. The full candidate-gene lists are browsable in the SoLD Shiny application. The contrast between the trials is not only a matter of how the candidate tables are counted. Genomic heritability, as estimated above, is higher under LIN, and that estimate involves no LD windows and no gene counting. More heritable traits are also easier to map, so the larger LIN candidate list follows from the heritability result rather than being independent evidence for it.
...
Both candidate lists were too large to inspect gene by gene, so we tested Gene Ontology (GO) biological-process and molecular-function terms for over-representation within each lipid class and trials, using a permutation null that preserves LD-block structure. This distinguishes terms supported by multiple independent intervals from those driven by a single LD block.

In total, 101 of 103 terms passed the LD-aware null ($q_{\mathrm{LD}}<0.05$). Because GO terms are nested and class sums and ratios share candidate genes, we merged terms with identical gene sets, leaving 50 term--gene-set combinations across 38 distinct terms (Supplementary Table~S7, loci collapsed). Of these, 21 draw on $\leq 30$ background genes and are shown in Fig.~\ref{fig:go_bp_main}; the rest were too broad to interpret (\textit{zinc ion binding}, 1{,}015 genes; \textit{transmembrane transport}, 526) and pointed to no lipid-specific process. Fifteen of the 21 are supported by two or more independent intervals, so most are not the product of a single LD block.

Those 21 terms are unevenly split between trials, with 19 under LIN and 2 under CTL, the same asymmetry that the candidate counts show. The two CTL terms are both related to tyrosine catabolism, derived from TG traits. The LIN terms centerr on nutrient transport and signaling, with cellular response to phosphate starvation and nitrate assimilation from SQDG and fatty-acid traits, along with sphingolipid metabolism and jasmonate-related defense regulation from AEG. They are carried mostly by SQDG and PC traits (8 and 5 of the 19), with fatty acids and TG contributing 3 each. Enrichment therefore narrows five thousand candidate genes to a short list which could be used for functional validation.
...
Gene   & Individual lipids   & 1100 & 4319 & 289 & 139.6 & 2.07 & 0.056 & $3.6\times10^{-35}$ \\
Gene   & Class sums / ratios &  115 &  812 &  18 &   2.7 & 6.56 & 0.020 & $2.9\times10^{-10}$ \\
\midrule
100 kb & Individual lipids   &  521 & 1562 & 193 & 168.0 & 1.15 & 0.102 & $8.0\times10^{-3}$ \\
100 kb & Class sums / ratios &   50 &  247 &   7 &   2.5 & 2.75 & 0.024 & $1.3\times10^{-2}$ \\
\textbf{250 kb} & \textbf{Individual lipids} & 378 & 970 & 168 & 160.9 & \textbf{1.04} & 0.142 & \textbf{0.23} \\
