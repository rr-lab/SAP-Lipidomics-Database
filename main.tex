...
Subsequently, we quantified genetic variance in the lipidome by estimating genomic heritability ($h^2$) for every species detected in each trial (214 under CTL, 216 under LIN) and compared the trials on the 163 species detected in both and measured across the same 351 genotypes (Supplementary Table~S2, per-species paired). Under CTL, the median $h^2$ was near zero, and only 11 of those species had heritability that we could distinguish from zero. Under LIN, the median was 0.170, and 53 species did. The same direction held for species by species comparison, with $h^2$ higher under LIN for 131 species, higher under CTL for 21, and equal for 11 (Supplementary~\ref{fig:suppfig_s4_heritability}), and for class by class comparison where every class was higher under LIN except TG and PA (Supplementary Table~S2, structure-conditioned). Two checks indicate that this is not an artifact of ancestry or of which genotypes were sampled. Fitting the three leading eigenvectors of the relationship matrix as fixed effects, which removes variation between genetic clusters, left 51 of the 66 LIN-heritable species still heritable (Supplementary Table~S2, structure-conditioned). Re-estimating after dropping each botanical race in turn and after twenty random 80\% subsamples of the panel gave 14 to 26 heritable species under CTL and 43 to 87 under LIN on the full per-trial sets. The two ranges never overlap, so LIN exceeded CTL in every run (Supplementary Table~S2, ancestry robustness). 
...
relationship matrix, the same 351 genotypes and the same 163 species detected
